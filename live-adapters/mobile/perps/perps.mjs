...
function colorMatches(actual, expected) {
  const a = normalizeCssColor(actual);
  const e = normalizeCssColor(expected);
  if (!e) return true;
  if (e === 'red') return ['red', '#f00', '#ff0000', 'rgb(255,0,0)', 'rgba(255,0,0,1)'].includes(a);
  if (e === 'white') return ['white', '#fff', '#ffffff', 'rgb(255,255,255)', 'rgba(255,255,255,1)'].includes(a);
  return a === e;
}
...
async function readMobileDebugBanner(input) {
  const testId = String(input.node?.test_id ?? input.node?.testID ?? 'adr58-btc-position-banner');
  const headerText = String(input.node?.above_text ?? input.node?.position_header_text ?? 'Your positions');
  const expression = `(function(){
    var testId = ${JSON.stringify(testId)};
    var headerText = ${JSON.stringify(headerText)};
    var hook = globalThis.__REACT_DEVTOOLS_GLOBAL_HOOK__;
    function appendText(value, out) {
      ...
      if (Array.isArray(value)) {
        value.forEach(function(entry){ appendText(entry, out); });
        return;
      }
      if (typeof value === 'object' && value && value.props) appendText(value.props.children, out);
    }
    function collectTexts(fiber, out) {
      if (!fiber) return;
      appendText(fiber.memoizedProps && fiber.memoizedProps.children, out);
      collectTexts(fiber.child, out);
      collectTexts(fiber.sibling, out);
    }
    function flattenStyle(style) {
      if (!style) return {};
      if (Array.isArray(style)) return style.reduce(function(acc, item){ return Object.assign(acc, flattenStyle(item)); }, {});
      if (typeof style === 'object') return style;
      return { unresolved: style };
    }
    var result = { present: false, testId: testId, text: null, bannerIndex: null, headerIndex: null, aboveHeader: null, bannerStyle: {}, textStyle: {}, headerText: headerText };
    var index = 0;
    function walk(fiber) {
      if (!fiber) return;
      var props = fiber.memoizedProps || {};
      var ownTexts = [];
      appendText(props.children, ownTexts);
      if (props.testID === testId) {
        var texts = [];
        collectTexts(fiber, texts);
        result.present = true;
        result.text = Array.from(new Set(texts)).join(' ').trim();
        result.bannerIndex = index;
        result.bannerStyle = flattenStyle(props.style);
        var childProps = fiber.child && fiber.child.memoizedProps || {};
        result.textStyle = flattenStyle(childProps.style);
      }
      if (result.headerIndex === null && ownTexts.some(function(text){ return text === headerText || text.indexOf(headerText) !== -1; })) {
        result.headerIndex = index;
      }
      index += 1;
      walk(fiber.child);
      walk(fiber.sibling);
    ...
    if (hook && hook.renderers && hook.getFiberRoots) {
      var it = hook.renderers.entries();
      var step = it.next();
      while (!step.done) {
        var roots = hook.getFiberRoots(step.value[0]);
        if (roots) roots.forEach(function(root){ if (root.current) walk(root.current); });
        step = it.next();
      }
    }
    if (result.bannerIndex !== null && result.headerIndex !== null) result.aboveHeader = result.bannerIndex < result.headerIndex;
    return JSON.stringify(result);
  })()`;
  return evalSync(input, expression);
}
...
export async function assertDebugBanner(input) {
  ...
  const expectedBackground = input.node?.background_color ?? input.node?.backgroundColor ?? 'red';
  const expectedTextColor = input.node?.text_color ?? input.node?.textColor ?? 'white';
  let last = null;
  while (Date.now() <= deadline) {
    last = await readMobileDebugBanner(input);
    if (!expectedPresent && !last.present) {
      return { action: input.action, expected: 'absent', present: false, assertion: 'pass', proofPath: 'mobile-react-native-fiber' };
    }
    if (expectedPresent && last.present) {
      const textOk = last.text === expectedText;
      const backgroundOk = colorMatches(last.bannerStyle?.backgroundColor, expectedBackground);
      const textColorOk = colorMatches(last.textStyle?.color, expectedTextColor);
      const placementOk = input.node?.require_placement === false ? true : last.aboveHeader === true;
      if (textOk && backgroundOk && textColorOk && placementOk) {
        return {
          action: input.action,
          expected: 'present',
          present: true,
          assertion: 'pass',
          text: last.text,
          style: { backgroundColor: last.bannerStyle?.backgroundColor ?? null, color: last.textStyle?.color ?? null },
          placement: { aboveHeader: last.aboveHeader, bannerIndex: last.bannerIndex, headerIndex: last.headerIndex, headerText: last.headerText },
          proofPath: 'mobile-react-native-fiber',
        };
      }
    }
    await sleep(250);
  }
  throw new Error(`Debug banner assertion failed after ${timeoutMs}ms: ${JSON.stringify({ expectedPresent, expectedText, expectedBackground, expectedTextColor, last })}`);
}
