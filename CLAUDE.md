...
This repo implements MetaMask domain support for Recipe Protocol v1.

Canonical protocol source of truth:

- https://farmslot.io/docs/reference/recipe-protocol-v1

Installed protocol/runtime packages provide the implementation. Use `FARMSLOT_ROOT`, `.farmslot-root`, or a sibling checkout only as an explicit local co-development override. Never hardcode a developer-specific absolute path.

## Hard rules

1. **Do not redefine Recipe Protocol v1 here.** This repo may document MetaMask-specific examples and adapter details, but field names, graph semantics, trace shape, artifact manifest shape, validation rules, and action/flow manifest rules come from the canonical Recipe Protocol v1 spec.
...
7. **Keep generic mechanics out of this runner.** Generic `ui.*`, CDP/RN transports, graph execution, trace, summary, artifact manifest, and protocol validation belong in the shared protocol/runtime packages, not duplicated here.
8. **Wrappers stay thin.** External wrappers should resolve/install/invoke this runner; they must not contain a second runner, copied harness, task-specific recipes, or graph executor.
...
- Does every manifest/catalog addition follow Recipe Protocol v1?
...
- Are generic capabilities left in the shared protocol/runtime packages instead of reimplemented here?
