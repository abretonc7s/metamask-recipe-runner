...
export async function startRecipeRecording(
  ...
  const child = spawn(captureHelperPath(), ['record', '--pid', String(pid), '--output', outputPath], {
    cwd: projectRoot,
    env: process.env,
...
export async function stopRecipeRecording(
  ...
  if (!fs.existsSync(recording.outputPath) || fs.statSync(recording.outputPath).size === 0) {
    console.error(
      `WARN: capture-helper recording did not produce a usable video artifact: ${recording.stderr || recording.stdout || recording.outputPath}`,
    );
    return;
  }
...
function addRecordingArtifactToManifest(result: RecipeRunResult, recording: ActiveRecipeRecording): void {
  ...
  if (manifest.artifacts.some((artifact) => artifact.path === recording.relativePath)) return;
