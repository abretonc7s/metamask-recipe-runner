#!/usr/bin/env bash
# inject-core-harness.sh — headless `core` adapter install.
#
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help) echo "Usage: inject-core-harness.sh [--target <metamask-core>]"; exit 0 ;;
...
CLEANUP_COMMAND="RECIPE_HARNESS_ROOT=$HARNESS_ROOT $(printf '%q' "$SCRIPT_DIR/cleanup-core-harness.sh") --target $(printf '%q' "$TARGET")"
node -e '
  const fs = require("fs");
