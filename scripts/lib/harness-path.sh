...
# defaults to temp/recipe/harness (under the gitignored temp/, so installs
...
# Override RECIPE_RUNTIME_DIR (relative to target repo); defaults to temp/recipe/runtime.
recipe_runtime_dir() {
  local root="${RECIPE_RUNTIME_DIR:-temp/recipe/runtime}"
  case "$root" in
